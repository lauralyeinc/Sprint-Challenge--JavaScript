...
console.log(TyrDinosuar.weight);

// What was the diet of a velociraptor?
console.log(veloDinosuar.diet);

// How long was a stegosaurus?
console.log(StegoDinosuar.length);

// What time period did tyrannosaurus live in?
console.log(TyrDinosuar.period);


// Create a new roar method for the tyrannosaurus.  When called, return "RAWERSRARARWERSARARARRRR!" Log the result.
TyrDinosuar.roar = function() {
  return `RAWERSRARARWERSARARARRRR!`;
}
console.log(TyrDinosuar.roar());
...
const uni = function(item) {
 const results = [];
 for(let i = 0; i < item.length; i++) {
   let containsUni = item[i].university;
   if(containsUni.includes("Uni")) {
     results.push(containsUni);
   }
 }
 return results;
};
...
let allCaps = [];
zooAnimals.forEach(animalinfo => {
  lowerCase.push(animalinfo.animal_name);
});
console.log(lowerCase.map(animalinfo => animalinfo.toLowerCase())); 
...
populationTotal = population.reduce(Total)
